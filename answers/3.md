...
1. Очередность выполнения:
  ...
  3). Создается переменная elem и ей присваивается пустой массив(accum) c индексом 2(item).
...
3. В переменной "elem" будет undefined, потому что elem присваивается пустой массив.
```